...
const handler = () => {
  const now = new Date();
  const times = suncalc.getTimes(now, service.config.latitude, service.config.longitude)

  const pastKeys = Object.keys(times)
  .sort((a, b) => times[a] - times[b])
  .filter(key => now > times[key]);

  const lastKey = (pastKeys.length)
    ? pastKeys[pastKeys.length - 1]
    : 'night';
  ...
  if (currentDarkState !== lastDarkState) {
    service.send('dark', {value: currentDarkState === 'dark'}, { retain: true });
    lastDarkState = currentDarkState;
  }

  if (currentNightState !== lastNightState) {
    service.send('night', {value: currentNightState === 'night'}, { retain: true });
